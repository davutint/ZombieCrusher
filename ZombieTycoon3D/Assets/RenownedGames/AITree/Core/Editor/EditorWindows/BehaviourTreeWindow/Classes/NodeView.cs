...
namespace RenownedGames.AITreeEditor
{
    internal abstract class NodeView : NodeElement, IDropTarget
    {
        ...
        public NodeView(BehaviourTreeGraph graph, Node node, string uxmlFile) : base(uxmlFile)
        {
            ...
            if(node is SubTree)
            {
                node.Started -= OnSubTreeStarted;
                node.Started += OnSubTreeStarted;
                node.StateChanged -= OnSubTreeStateChanged;
                node.StateChanged += OnSubTreeStateChanged;
            }
        }
        ...
        #region [Callbacks]
        /// <summary>
        /// Called when sub tree node started.
        /// </summary>
        private void OnSubTreeStarted()
        {
            if (!graph.GetWindow().AutoLiveLink())
            {
                return;
            }
            ...
            {
                graph.GetWindow().SubTreeIn(subTree.GetSubTree());
            }
        }

        /// <summary>
        /// Called when sub tree node changed state.
        /// </summary>
        /// <param name="state">New state.</param>
        private void OnSubTreeStateChanged(State state)
        {
            if (!graph.GetWindow().AutoLiveLink())
            {
                return;
