...
namespace ArcadeVP
{
    public class ArcadeVehicleController : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            {
                //turnlogic
                var sign = Mathf.Sign(carVelocity.z);
                var TurnMultiplyer = turnCurve.Evaluate(carVelocity.magnitude / MaxSpeed);
        ...
        public void Visuals()
        {
            ...
            foreach (var FW in FrontWheels)
            {
                FW.localRotation = Quaternion.Slerp(FW.localRotation,
                    Quaternion.Euler(FW.localRotation.eulerAngles.x, 30 * steeringInput,
                        FW.localRotation.eulerAngles.z), 0.7f * Time.deltaTime / Time.fixedDeltaTime);
            ...
            if (carVelocity.z > 1)
            {
                BodyMesh.localRotation = Quaternion.Slerp(BodyMesh.localRotation, Quaternion.Euler(
                        ...
                        BodyMesh.localRotation.eulerAngles.y, BodyTilt * steeringInput),
                    0.4f * Time.deltaTime / Time.fixedDeltaTime);
            }
            else
            {
                BodyMesh.localRotation = Quaternion.Slerp(BodyMesh.localRotation, Quaternion.Euler(0, 0, 0),
                    0.4f * Time.deltaTime / Time.fixedDeltaTime);
            }
